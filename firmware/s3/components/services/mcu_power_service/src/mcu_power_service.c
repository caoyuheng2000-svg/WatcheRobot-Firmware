...
esp_err_t mcu_power_set_5v_enabled(bool enabled, mcu_power_source_t source, uint32_t *out_seq)
{
    ...
    ret = mcu_link_send_frame(link,
                              MCU_FRAME_CLASS_POWER,
                              enabled ? MCU_POWER_MSG_5V_ENABLE : MCU_POWER_MSG_5V_DISABLE,
                              MCU_FRAME_FLAG_ACK_REQ,
                              payload,
    ...
    ESP_LOGI(TAG, "Queued POWER_5V_%s frame seq=%lu wire_len=%u source=%u",
             enabled ? "ENABLE" : "DISABLE", (unsigned long)seq, (unsigned)wire_len, (unsigned)source);
    ESP_LOGI(OBS_TAG, "evt=power_5v_request seq=%lu msg_class=%u msg_id=%u enabled=%u source=%u",
             (unsigned long)seq,
             (unsigned)MCU_FRAME_CLASS_POWER,
             (unsigned)(enabled ? MCU_POWER_MSG_5V_ENABLE : MCU_POWER_MSG_5V_DISABLE),
             enabled ? 1u : 0u,
             (unsigned)source);
    ...
    switch (event->type) {
        case MCU_LINK_RX_EVENT_ACK:
            ...
            if (s_command_inflight && ref_seq == s_last_command_seq) {
                ESP_LOGI(TAG, "POWER ACK ref_seq=%lu status=%u enabled=%u source=%u",
                         (unsigned long)ref_seq,
                         (unsigned)decode_u16_le(&event->frame.payload[4]),
                         s_last_enabled ? 1u : 0u,
        ...
        case MCU_LINK_RX_EVENT_FAULT:
            ref_seq = decode_u32_le(event->frame.payload);
            if (event->frame.payload[4] == 0x07u && (!s_command_inflight || ref_seq == s_last_command_seq || ref_seq == 0u)) {
